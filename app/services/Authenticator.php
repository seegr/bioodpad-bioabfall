...
class Authenticator implements \Nette\Security\Authenticator
{
    ...
    public function authenticate(string $username, string $password): IIdentity
    {
        ...
        $row = $this->database->table(UserRepository::TABLE_NAME)
            ->select("user.*, organization.title AS orgTitle")
            ->where(Utils::createChainPath(UserRepository::TABLE_NAME, UserRepository::COLUMN_EMAIL), $username)
            ->fetch();

        $orgTitle = $row->orgTitle ?? null;
