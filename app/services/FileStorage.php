...
class FileStorage
{
    ...
    public function uploadImage(Image $image, string $key, string $extension = 'jpg'): string
    {
        if (!$this->namespace) {
        ...
        $uniqueName = $this->getRandomName($key . '.' . $extension);
    ...
    public function uploadBase64Image(string $string, string $key): ?string
    {
        ...
        $extension = explode('/', mime_content_type($string))[1];
